...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // validate_form()이 오류 메시지를 반환하면 show_form()으로 전달
...
function process_form($input) {
    ...
    // 주문 메시지 텍스트 생성
    $message = <<<_ORDER_
        주문이 완료되었습니다, {$input["name"]}님.<br>
        $sweet({$input["size"]}), $main_dish_1, $main_dish_2 을(를) 주문하셨습니다.<br>
        배달 여부: $delivery<br>
    ...
    // 주방장에게 메시지 보내기
    mail("killemx@hotmail.com", "새로운 주문", $message);
    // HTML 엔터티 인코딩 후 메시지를 출력하고
    // 줄 바꿈은 <br> 태크로 변경한다. 
